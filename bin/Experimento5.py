...
if __name__ == '__main__':
    ...
    for TRAIN_SIZE in range(100, 6000, 400):
        text_train, label_train, text_test, label_test = get_instances(df,TRAIN_SIZE,500)
        X_train, y_train, X_test, y_test = vectorizar(text_train, label_train, text_test, label_test)

        MAX_K_ACC = 0.0
        for K in range(50, 400, 50):
            MAX_K_ACC = max(MAX_K_ACC, run_test(df,K = K, TRAIN_SIZE = TRAIN_SIZE))
        MAX_KS_ACC.append(MAX_K_ACC)

    fout = open("resultados_exp5.pkl","wb")
    pickle.dump((MAX_KS_ACC),fout)
    fout.close()
